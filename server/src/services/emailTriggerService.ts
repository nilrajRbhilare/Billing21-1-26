...
export class EmailGuard {
  static async validate(trigger: CreateEmailTriggerRequest, context: any): Promise<{ allowed: boolean, reason?: string }> {
    ...
    } else if (trigger.vendorId) {
      const vendors = readCoreData('vendors.json', 'vendors');
      const vendor = vendors.find((v: any) => String(v.id) === String(trigger.vendorId));
      if (!vendor) return { allowed: false, reason: 'Vendor not found' };
      if (vendor.status === 'inactive') return { allowed: false, reason: 'Vendor is inactive' };
    } else {
      return { allowed: false, reason: 'Neither customer nor vendor specified' };
    }
  ...
  private static async processImmediateSend(triggerId: string, request: CreateEmailTriggerRequest, context: any) {
    try {
      ...
      if (request.customerId) {
        ...
        toEmails = recipients.length > 0 ? recipients.map((r: any) => r.email) : request.recipients.filter((r: any) => r.includes('@'));

        const customers = readCoreData('customers.json', 'customers');
        entityInfo = customers.find((c: any) => String(c.id) === String(request.customerId));
        entityName = entityInfo?.displayName || entityInfo?.name || 'Customer';
      } else if (request.vendorId) {
        // For vendors, we might not have contact persons in the same way, or we use the vendor email directly
        toEmails = request.recipients.filter((r: any) => r.includes('@'));
        const vendors = readCoreData('vendors.json', 'vendors');
        entityInfo = vendors.find((v: any) => String(v.id) === String(request.vendorId));
        entityName = entityInfo?.displayName || entityInfo?.name || 'Vendor';
      }

      if (toEmails.length === 0) {
        if (entityInfo && entityInfo.email) {
          toEmails.push(entityInfo.email);
        } else {
          console.error('[EmailTriggerService] No valid recipient email addresses found for entity:', request.customerId || request.vendorId);
      ...
      const content = await EmailTemplateRenderer.render({
        ...
        context: {
          ...
          customer: context.customer || (request.customerId ? entityInfo : null),
          vendor: context.vendor || (request.vendorId ? entityInfo : null),
