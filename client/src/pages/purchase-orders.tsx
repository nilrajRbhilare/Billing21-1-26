...
export default function PurchaseOrders() {
  ...
  const { currentPage, totalPages, totalItems, itemsPerPage, paginatedItems: rawPaginatedItems, goToPage } = usePagination<PurchaseOrder>(filteredPOs, 10);
  const displayPOs = (rawPaginatedItems || []) as PurchaseOrder[];
                          ...
                          <thead className="bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
                            <tr>
                              <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider w-10">
                                <Checkbox
                                  checked={selectedPOs.length === displayPOs.length && displayPOs.length > 0}
                                  onCheckedChange={(checked) => {
                                    if (checked) {
                                      setSelectedPOs(displayPOs.map(po => po.id));
                                    } else {
                                      setSelectedPOs([]);
                                    }
                                  }}
