...
export default function CustomerCreate() {
  ...
  const [activeTab, setActiveTab] = useState("other_details");
  ...
  const tabFields = {
    other_details: ["taxPreference", "gstTreatment", "placeOfSupply", "exemptionReason", "currency", "paymentTerms"],
    address: ["billingAddress", "shippingAddress"],
    contact_persons: ["contactPersons"]
  };

  const getTabErrors = (tab: keyof typeof tabFields) => {
    const fields = tabFields[tab];
    return fields.some(field => {
      const error = form.formState.errors[field as keyof CustomerFormValues];
      return !!error;
    });
  };
  ...
  useEffect(() => {
    if (form.formState.isSubmitted && !form.formState.isValid) {
      ...
      const tabs: (keyof typeof tabFields)[] = ["other_details", "address", "contact_persons"];
                ...
                <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                  <div className="border-b border-slate-100 bg-slate-50/50 px-6">
                    <TabsList className="w-full justify-start h-auto bg-transparent p-0 rounded-none gap-6">
                      <TabsTrigger
                        value="other_details"
                        className="rounded-none border-b-2 border-transparent px-2 py-3 data-[state=active]:border-blue-600 data-[state=active]:text-blue-600 data-[state=active]:bg-transparent data-[state=active]:shadow-none shadow-none focus-visible:ring-0 focus-visible:ring-offset-0 bg-transparent hover:bg-transparent transition-none font-medium text-sm text-slate-500 relative"
                      >
                        Other Details
                        {getTabErrors("other_details") && (
                          <span className="absolute top-2 right-0 h-2 w-2 rounded-full bg-red-500 border-2 border-white" />
                        )}
                      </TabsTrigger>
                      <TabsTrigger
                        value="address"
